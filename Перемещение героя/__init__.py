...
def check_coord(player):
    ...
    x = player.rect.x
    y = player.rect.y
    print("rewrwrew", level_map[mesto_y][mesto_x])
    if vel_x > 0 and vel_x < size[0] and vel_y > 0 and vel_y < size[1] and (
            level_map[mesto_y][mesto_x][0] == '.' or level_map[mesto_y][mesto_x][0] == '@'):
        player.rect.x = vel_x
        player.rect.y = vel_y
...
class Camera:
    ...
    def update(self, target):
        ...
        global y
        global flag
        mesto_y = ((x + v_x) // tile_height)
        mesto_x = ((y + v_y) // tile_width)
        if level_map[mesto_y][mesto_x][0] != "#" and x > 0 and x < size[0] and y > 0 and y < size[1]:
            player.rect.x += v_x
            player.rect.y += v_y
            x += v_x
            y += v_y
